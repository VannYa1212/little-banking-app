...
const PassCodeKeyboard = ({ onPress }:any) => {
  return (
    <View
      style={{
        flexDirection: 'row',
...
const PassCode = ({ passcode, isValid }:any) => {
  return (
    <MotiView
      animate={{
        translateX: isValid
...
export default function PassCodeV1() {
  const [passcode, setPasscode] = React.useState<any>([]);
    ...
    <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
      ...
      <PassCodeKeyboard
        onPress={(char:any) => {
          ...
          setPasscode((passcode:any) => newPasscode);
        }}
      />
    </View>
  );
